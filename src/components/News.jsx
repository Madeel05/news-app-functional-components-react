...
const News = (props) => {
    ...
    const fetchMoreData = async () => {
        ...
        const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=c3bfd6779ee040558d8979ae018ba646&page=${page + 1}&pageSize=${props.pageSize}`;
        setPage(page + 1)
        let data = await fetch(url);
        let pasrseData = await data.json();
    ...
    return (
        <>
            <h2 className='mx-5' style={{
                marginTop: "90px"
            }}>News App - Top {capitalizeFirstLetter(props.category)} Headlines</h2>
            <InfiniteScroll
